...
private fun Toast(
    ...
    Surface(
        ...
        Row(
            ...
            Text(message, color = textColor, style = MaterialTheme.typography.labelMedium)
        }
    }
}
...
class ToastUIState {
    ...
    fun applyShow(
        ...
        scope.launch {
            requestFlow.emit(ToastRequest(ToastModel(message, icon, type = type), onlyOne, dismissedListener))
        }
    }
...
fun <R> Result<R>.toastAutoApply(toastUIState: ToastUIState, actionName: String) {
    onSuccess {
        toastUIState.applyShow(actionName+"成功", ToastModel.Type.Success)
    }
    onFailure {
        toastUIState.applyShowForError(it)
    }
}